...
st.write('<b><u>Note de lecture : </b></u>Le revenu disponible par uniré de consommation correspond au niveau de vie d un ménage. Par exemple, dans la cour d’appel de ', data_med_disp['ca'].iloc[0].title(),', le revenu disponible par unité de consommation est de ',round(data_med_disp[pop_options[selected_pop]].iloc[0]),'€, contre ',round(df_rev_disp[df_rev_disp['ca'].isin(liste_ca)][pop_options[selected_pop]].mean()),'€ dans la population Française.', unsafe_allow_html=True)
...
st.write('<b><u>Note de lecture :</b></u> À la cour d’appel de',filtered_df_cluster['ressort_ca'].iloc[0].title(),',', round(filtered_df_cluster['N_min_vie'].iloc[0]),'personnes sur 100.000 bénéficient du minimum vieillesse. Sur l’ensemble de la population Française, ce ratio est de',round(df_cluster[df_cluster['ressort_ca'].isin(liste_ca)].N_min_vie.mean()),'personnes sur 100.000.', unsafe_allow_html=True)
...
st.write('<b><u>Note de lecture :</b></u> L’intensité de la pauvreté est définie comme étant l’écart relatif entre le revenu moyen des personnes pauvres et le seuil de pauvreté. En France et en Europe, le seuil est le plus souvent fixé à 60 % du niveau de vie médian. Plus cet indicateur est élevé et plus la pauvreté est dite intense. Au sein de la cour d’appel de ',df_intens_pauv[df_intens_pauv['ca'].isin(selected_ca)].iloc[0].ca.title(),', ce seuil est de ', round(df_intens_pauv[df_intens_pauv['ca'].isin(selected_ca)].iloc[0].intens_pauv, 3),', contre',round(df_intens_pauv[df_intens_pauv['ca'].isin(liste_ca)].intens_pauv.mean(), 3),'dans l’ensemble de la population Française.',unsafe_allow_html=True)
